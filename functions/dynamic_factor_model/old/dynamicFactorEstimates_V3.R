dynamicFactorsEstimates <- function(X, q, lag.max = 5){
    ...
    s <- 8
    F <- bindLags(f.hat, s, na.omit = TRUE, bind.original = TRUE)
    ...
    A.hat <- cbind(A.hat, matrix(0, q, (s-p) * q))
    A.hat <- rbind(A.hat, cbind(diag(s-1) %x% diag(q), matrix(0, (s-1) * q, q)))
    
    
    A.hat <- rbind(A.hat, diag(q) %x% diag(p))
    A.hat <- c
