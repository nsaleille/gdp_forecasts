...
for (h in 1:8){
  ...
  fs.trim[[paste('h =', h)]] <- dynamicFactorsEstimatesVec(Xs, q, s = s, aggregate = FALSE)
  print(paste('Dynamic factors estimates for h =', h,': Done'))
}

save(ys, file = '../data/data_clean/outcome_sets.RData')
save(fs.trim, file = '../data/factors_estimates/factors_monthly.RData')