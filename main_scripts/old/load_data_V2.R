...
for (i in paths){
  ...
  info <- sapply(series, function(x) c(strsplit(x$name, '...', fixed = TRUE), 
                                       head(index(x$zoo), 1), 
                                       tail(index(x$zoo), 1), x$freq, length(x$zoo)))
  series.info <- rbind(series.info, unlist(info))
}
...
print(xtable(t(series.info)), file = '../tex/series_info.tex')
